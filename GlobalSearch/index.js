...
(function () {
  ...
  function GlobalSearchModal({ visible, onClose }) {
    ...
    async function runSearch() {
      ...
      // Dla 100 serwerów Discord zazwyczaj rate limituje do ~5 równoległych,
      // reszta czeka w retry — działa automatycznie.
  ...
  function start() {
    // Szukamy komponentów które mogą być głównym ekranem aplikacji
    const candidates = [
    ...
    // Sprawdź też przez findByProps
    const propCandidates = [
      ["rootNavigator"], ["appLayout"], ["layerScene"],
      ["renderRootScene"], ["renderApp"],
    ];
    for (const props of propCandidates) {
      const m = findByProps(...props);
      if (m) found.push("props:" + props[0]);
    }

    // Pokaż toast z tym co znaleziono
    ...
    // Spróbuj patchować każdy znaleziony komponent przez nazwę
    for (const name of candidates) {
      ...
      try {
        const p = after("render", target.prototype ?? target, (_args, res) => {
          ...
          return React.createElement(
            ...
            { style: { flex: 1 } },
            res,
            React.createElement(GlobalSearchRoot, { key: "gs-root-" + name }),
            React.createElement(
              TouchableOpacity,
              {
                key: "gs-fab-" + name,
        ...
        // ignoruj błędy patchowania
      }
    }

    console.log("[GlobalSearch] started, found:", found.join(", "), "patches:", patches.length);
  }
